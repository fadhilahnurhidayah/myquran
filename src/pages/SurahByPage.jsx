          ...
          <div className="flex justify-center items-center gap-3">
            ...
            <input
              ...
              className={`w-16 ${darkMode 
                ...
                rounded px-2 py-1 text-center shadow-inner border`}
              min={1}
              max={604}
            />
        ...
        {loading ? (
          ...
          <div className="space-y-6 w-full mx-auto">
            {verses.map((verse) => (
              <div 
                key={verse.id} 
                className={`rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border ${darkMode 
                  ? 'bg-gray-800 bg-opacity-80 text-white border-gray-700' 
                  : 'bg-white bg-opacity-90 text-gray-800 border-[#D6C6AF]'}`}
              >
                <p className={`text-right text-[1.8rem] font-amiri leading-loose ${darkMode ? 'text-amber-200' : 'text-[#7D5A3C]'}`}>
                  {verse.text_uthmani}
                </p>
